...
def _calculate_forces_numba_parallel(positions, masses, G, epsilon, forces):
    """
    Numba-jitted function for parallel force calculation.
    """
    ...
    for i in prange(num_particles):
        for j in range(i + 1, num_particles):
            ...
            # Use atomic adds for thread safety, though for this loop structure it's not strictly necessary
            # as each 'i' is independent. However, if 'j' loop was parallel, it would be.
            forces[i, 0] += fx
            forces[i, 1] += fy
            forces[i, 2] += fz
...
def calculate_forces(positions, masses, G, epsilon, **kwargs):
    """
    Wrapper for the Numba parallel kernel to include timing and metrics.
    """
    num_particles = positions.shape[0]
    forces = np.zeros_like(positions)

    start_time = time.time()
    _calculate_forces_numba_parallel(positions, masses, G, epsilon, forces)
